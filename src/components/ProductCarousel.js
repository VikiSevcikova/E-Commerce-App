...
const ProductCarousel = ({previewImage, detailImages}) => {
    return(
        <div id="productCarousel" className="carousel slide" data-ride="carousel" data-bs-interval="false">
            <div className="carousel-indicators">
                <a type="button" data-bs-target="#productCarousel" data-bs-slide-to="0" className="active" aria-current="true" aria-label="Slide 0">
                ...
                {detailImages.map((img, i)=>
                    <a key={`btn-${img.id}`} type="button" data-bs-target="#productCarousel" data-bs-slide-to={i+1} aria-label={`Slide ${i+1}`}>
                        <img src={img.imgUrl} className="d-block w-100" alt={img.description}/>
                    </a>
                )}
            </div>
